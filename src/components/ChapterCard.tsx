...
export function ChapterRow({
  ...
  return (
    <button
      onClick={onPress}
      className="mb-3 w-full overflow-hidden rounded-2xl border border-border bg-surface
    ...
    >
      ...
      {photos.length === 0 && (
        <div
          className="flex h-20 w-full items-center bg-gradient-to-r from-plum/40 to-surface
                        px-5"
        >
          <p className="truncate font-serif text-sm italic text-accent/60">
            {chapter.contributions.find((contribution) => contribution.type === "quote")
            ...
            {chapter.contributions
              .filter(
                (contribution, index, array) =>
                  array.findIndex((item) => item.author.id === contribution.author.id) ===
                  index,
              )
              .slice(0, 4)
